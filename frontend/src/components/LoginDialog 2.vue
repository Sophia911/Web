<template>
          ...
          <el-tab-pane label="登录">
            <el-form label-position="right" label-width="80px" class="login-form">
              ...
              <el-row justify="center">
                <el-button @click="login" type="primary" :disabled="!(emailOK && password && isAgree)" >
                  登录
                </el-button>
              </el-row>
            </el-form>
          ...
          <el-tab-pane label="注册">
            <el-form label-position="right" label-width="100px" class="login-form">
              ...
              <el-form-item label="验证码：">
                <el-row>
                  <el-col :span="22"><el-input type="password" v-model="identifyCode"/></el-col>
                  <el-col :span="2">
                    <el-button type="primary" @click="getIdentifyCode" plain>获取验证码</el-button>
                  </el-col>
                </el-row>
              </el-form-item>
              <el-form-item>
  ...
  export default {
    setup(props, context) {
      ...
      function login() {
        console.log("登录中...");
        axios.post(url + "/user/per", {
            id: loginForm.email
        }).then(response => {
          const pwd = response.data.data
          if (pwd === null) {// 未注册的邮箱号
            tipToRegister()
          } else if (pwd === loginForm.password) {
