<template>
    <div class="body">
        <div class="Function-Menu">
            ...
            <div v-for="inform in this.MenuItem.inform" :key="inform"
            class="Function-item" 
            @click="choseItem(inform)"
            @mouseover="focusItem(inform)"
            @mouseleave="blurItem(inform)"
            :style=inform.color>
                {{inform.name}}
            </div>
        </div>
        <div class="Function-Operation-Container">
            <div class="Operation">
                <ManageTheme v-if="this.MenuItem.Mamanage[0]" :op="this.$data.themeOp" @askForShowConfirmTable="childAskForShowConfirmTable"></ManageTheme>
                <ManageVideo v-if="this.MenuItem.Mamanage[1]" :op="this.$data.themeOp" @askForShowConfirmTable="childAskForShowConfirmTable"></ManageVideo>
                <ManagePersonPage v-if="this.MenuItem.Mamanage[2]" :op="this.$data.themeOp" @askForShowConfirmTable="childAskForShowConfirmTable"></ManagePersonPage>
                <MamanageAbout v-if="this.MenuItem.Mamanage[3]" :op="this.$data.themeOp" @askForShowConfirmTable="childAskForShowConfirmTable"></MamanageAbout>
             </div>
        </div>
        <div class="confirm-table" v-show="$data.showConfirmTable">
            <div class="op-befor-submission">
                ...
                <div class="befor-submission-item">
                    <input type="text" v-model=this.$data.VerificationCode class="input-verification-code"  placeholder="输入验证码">
                    <h5 class="get-verification-code" v-if="!this.$data.getVerificationCode"
                    @click="userGetVerificationCode"
                    @mousemove="focusUserGetVerificationCode"
                    @mouseleave="blurUserGetVerificationCode"
                    :style="this.$data.getVerificationCodeColor"
                    >点击以获取验证码</h5>
                    <h5 class="get-verification-code" v-if="this.$data.getVerificationCode">已发送{{this.$data.validTime}}s内有效</h5>
                </div>
            </div>
            <div class="op-button">
                <div class="submission" 
                @click="confirmSubmission" 
                @mousemove="focusSubmission('确认')"
                @mouseleave="blurSubmission('确认')"
                :style=this.$data.confirmSubmitColor>确认提交</div>
                <div class="submission" 
                @click="cancelSubmission" 
                @mousemove="focusSubmission('取消')"
                @mouseleave="blurSubmission('取消')"
                :style=this.$data.cancelSubmitColor>取消提交</div>
            </div>
        </div>
        <div class="Function-Options-Container" v-if="this.MenuItem.Options[this.$data.currentItem]">
            <div class="show-hiden" @click="showHidenFOC"><div :class=$data.showFunctionOptionsClass>
            </div></div>
            <div class="Function-Options" v-show="$data.showFunctionOptions">
            <div class="Function-title">
                {{this.MenuItem.Options[this.$data.currentItem].title}}                
            </div>
            <div v-for="op in this.MenuItem.Options[this.$data.currentItem].op" :key="op" 
            class="Function-item"
            @click="choseFunction(op)"
            @mouseover="focusFunction(op)"
            @mouseleave="blurFunction(op)"
            :style=op.color>
                {{op.name}}
            </div>
        </div>
